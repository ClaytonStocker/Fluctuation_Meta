...
system.time( #  34ish minutes
  if(run){
    Overall_Model_CVR <- metafor::rma.mv(InCVR ~ 1, V = v_InCVR, test = "t", dfs = "contain",
...
system.time( #  14ish minutes
  if(run){
    Amplitude_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                           mods = ~ T2_Magnitude - 1,
                                           random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                         ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                           R = list(phylo=A_cor), data = data, method = "REML", sparse = TRUE, 
...
Amplitude_Plot_CVR <- ggplot(Plot_Data, aes(x = T2_Magnitude, y = InCVR)) + 
                      ...
                      labs(x = "Fluctuation Amplitude (\u00B0C)", y = "Effect Size (InCVR)", 
...
system.time( #  29ish minutes
  if(run){
    Fluctuation_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
...
density_fluctuation_CVR <- fluctuation_table %>% mutate(name = fct_relevel(name, Fluctuation_Order)) %>%
                           ...
                           labs(x = TeX("Effect Size (InCVR)"), y = "") +
...
system.time( #  43ish minutes
  if(run){
    Trait_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
...
system.time( #  23ish minutes
  if(run){
    Class_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                       mods = ~ Class - 1,
                                       random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                     ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  17ish minutes
  if(run){
    Specific_Trait_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                mods = ~ Measurement - 1,
                                                random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                              ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Population_Model_CVR <- metafor::rma.mv(InCVR ~ 1, V = v_InCVR, test = "t", dfs = "contain",
                                            random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                          ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Population_Amplitude_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                      mods = ~ T2_Magnitude - 1,
                                                      random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                    ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                                      R = list(phylo=Population_A_cor), data = Population_Subset_Data, method = "REML", sparse = TRUE, 
...
system.time( #  1ish minutes
  if(run){
    Population_Fluctuation_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                        mods = ~ Fluctuation_Category - 1,
                                                        random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                      ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Population_Class_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                  mods = ~ Class - 1,
                                                  random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  31ish minutes
  if(run){
    Individual_Model_CVR <- metafor::rma.mv(InCVR ~ 1, V = v_InCVR, test = "t", dfs = "contain",
                                            random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                          ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  19ish minutes
  if(run){
    Individual_Amplitude_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                      mods = ~ T2_Magnitude - 1,
                                                      random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                    ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                                      R = list(phylo=Individual_A_cor), data = Individual_Subset_Data, method = "REML", sparse = TRUE, 
...
system.time( #  20ish minutes
  if(run){
    Individual_Fluctuation_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                        mods = ~ Fluctuation_Category - 1,
                                                        random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                      ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  12ish minutes
  if(run){
    Individual_Class_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                  mods = ~ Class - 1,
                                                  random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Aquatic_Model_CVR <- metafor::rma.mv(InCVR ~ 1, V = v_InCVR, test = "t", dfs = "contain",
                                         random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                       ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Aquatic_Amplitude_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                   mods = ~ T2_Magnitude - 1,
                                                   random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                 ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                                   R = list(phylo=Aquatic_A_cor), data = Aquatic_Subset_Data, method = "REML", sparse = TRUE, 
...
system.time( #  1ish minutes
  if(run){
    Aquatic_Fluctuation_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                     mods = ~ Fluctuation_Category - 1,
                                                     random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                   ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Aquatic_Trait_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                               mods = ~ Trait_Category - 1,
                                               random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                             ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                               R = list(phylo=Aquatic_A_cor), data = Aquatic_Subset_Data, method = "REML", sparse = TRUE, 
...
density_aquatic_trait_CVR <- aquatic_trait_table %>% mutate(name = fct_relevel(name, Aquatic_Trait_Order)) %>%
                             ...
                             label= paste("italic(k)==", c(aquatic_trait_table["Physiological", "K"], 
                                                         ...
                                                         c(aquatic_trait_table["Physiological", "K"], 
                                                           aquatic_trait_table["Morphology", "K"], 
                                                           aquatic_trait_table["Life-history Traits", "K"], 
...
system.time( #  1ish minutes
  if(run){
    Aquatic_Plasticity_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
...
system.time( #  1ish minutes
  if(run){
    Aquatic_Specific_Trait_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                        mods = ~ Measurement - 1,
                                                        random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                      ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
Aquatic_Specific_Trait_Order <- c("Metabolic Rate", "Mass", "Locomotor Performance", "Length", 
                                  "Immune Defense", "Food Consumption", "Development Time", "Cortisol Levels", "Apparent Digestability Coefficient")
...
system.time( #  8ish minutes
  if(run){
    Terrestrial_Model_CVR <- metafor::rma.mv(InCVR ~ 1, V = v_InCVR, test = "t", dfs = "contain",
                                             random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                           ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  3ish minutes
  if(run){
    Terrestrial_Amplitude_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                       mods = ~ T2_Magnitude - 1,
                                                       random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                     ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                                       R = list(phylo=Terrestrial_A_cor), data = Terrestrial_Subset_Data, method = "REML", sparse = TRUE, 
...
system.time( #  6ish minutes
  if(run){
    Terrestrial_Fluctuation_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                         mods = ~ Fluctuation_Category - 1,
                                                         random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                       ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  8ish minutes
  if(run){
    Terrestrial_Trait_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                   mods = ~ Trait_Category - 1,
                                                   random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                 ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                                   R = list(phylo=Terrestrial_A_cor), data = Terrestrial_Subset_Data, method = "REML", sparse = TRUE, 
...
system.time( #  8ish minutes
  if(run){
    Terrestrial_Plasticity_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                        mods = ~ Plasticity_Mechanism - 1,
                                                        random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                      ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                                        R = list(phylo=Terrestrial_A_cor), data = Terrestrial_Subset_Data, method = "REML", sparse = TRUE, 
...
system.time( #  3ish minutes
  if(run){
    Terrestrial_Specific_Trait_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                            mods = ~ Measurement - 1,
                                                            random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                          ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Acclimation_Model_CVR <- metafor::rma.mv(InCVR ~ 1, V = v_InCVR, test = "t", dfs = "contain",
                                             random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                           ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Acclimation_Amplitude_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                       mods = ~ T2_Magnitude - 1,
                                                       random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                     ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                                       R = list(phylo=Acclimation_A_cor), data = Acclimation_Subset_Data, method = "REML", sparse = TRUE, 
...
system.time( #  1ish minutes
  if(run){
    Acclimation_Exposure_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
...
Acclimation_Exposure_Amplitude_Plot_CVR <- ggplot(Acclimation_Exposure_Plot_Data, aes(x = Acclimation_Exposure_Time, y = InCVR)) + 
                                           geom_point(aes(x = Acclimation_Exposure_Time, y = InCVR, 
                                                      size = fct_relevel(n_category, c("25", "50", "75", "> 75"))), 
                                                      shape = 21, fill = "#4292c6", alpha = 0.5) + 
                                           labs(x = "Exposure Time (Days)", y = "Effect Size (InCVR)", 
                                                size = "Sample Size", title = "Acclimation Treatments") +
                                           theme_bw() +
                                           theme(plot.title = element_text(size = 12, colour ="black", face = "bold", hjust = 0.5, margin = margin(b = 10))) +
                                           theme(axis.text.y = element_text(size = 10, colour ="black", margin = margin(l = 5))) +
...
system.time( #  1ish minutes
  if(run){
    Acclimation_Frequency_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
...
Acclimation_Frequency_Plot_CVR <- ggplot(Acclimation_Frequency_Plot_Data, aes(x = Number_Of_Fluctuations, y = InCVR)) + 
                                  geom_point(aes(x = Number_Of_Fluctuations, y = InCVR, 
                                             size = fct_relevel(n_category, c("25", "50", "75", "> 75"))), 
                                             shape = 21, fill = "#4292c6", alpha = 0.5) + 
                                  labs(x = "Number of Fluctuations", y = "Effect Size (InCVR)", 
                                       size = "Sample Size", title = "Acclimation Treatments") +
                                  theme_bw() +
                                  theme(plot.title = element_text(size = 12, colour ="black", face = "bold", hjust = 0.5, margin = margin(b = 10))) +
                                  theme(axis.text.y = element_text(size = 10, colour ="black", margin = margin(l = 5))) +
...
system.time( #  1ish minutes
  if(run){
    Acclimation_Fluctuation_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                         mods = ~ Fluctuation_Category - 1,
                                                         random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                       ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Acclimation_Trait_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                   mods = ~ Trait_Category - 1,
                                                   random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                 ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Acclimation_Stage_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
...
system.time( #  1ish minutes
  if(run){
    Acclimation_Class_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                   mods = ~ Class - 1,
                                                   random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                 ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  1ish minutes
  if(run){
    Acclimation_Specific_Trait_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                            mods = ~ Measurement - 1,
                                                            random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                          ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  12ish minutes
  if(run){
    Developmental_Model_CVR <- metafor::rma.mv(InCVR ~ 1, V = v_InCVR, test = "t", dfs = "contain",
                                               random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                             ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  4ish minutes
  if(run){
    Developmental_Amplitude_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                         mods = ~ T2_Magnitude - 1,
                                                         random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                       ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                                         R = list(phylo=Developmental_A_cor), data = Developmental_Subset_Data, method = "REML", sparse = TRUE, 
...
system.time( #  11ish minutes
  if(run){
    Developmental_Fluctuation_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                           mods = ~ Fluctuation_Category - 1,
                                                           random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                         ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  12ish minutes
  if(run){
    Developmental_Trait_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                     mods = ~ Trait_Category - 1,
                                                     random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                   ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
                                                     R = list(phylo=Developmental_A_cor), data = Developmental_Subset_Data, method = "REML", sparse = TRUE, 
...
system.time( #  13ish minutes
  if(run){
    Developmental_Exposure_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
...
system.time( #  6ish minutes
  if(run){
    Developmental_Class_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                     mods = ~ Class - 1,
                                                     random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                   ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
system.time( #  5ish minutes
  if(run){
    Developmental_Specific_Trait_Model_CVR <- metafor::rma.mv(InCVR, V = v_InCVR, test = "t", dfs = "contain",
                                                              mods = ~ Measurement - 1,
                                                              random = list(~1|phylo, ~1|Study_ID, ~1|obs,
                                                                            ~1|Shared_Animal_Number, ~1|Shared_Control_Number), 
...
Raw_Specific_Trait_CVR <- data.frame("Specific Phenotypic Traits" = c("Apparent Digestability Coefficient", "Catalase Activity", "Cortisol Levels", "Development Time", "Fecundity", 
...
Raw_Aquatic_Specific_Trait_CVR <- data.frame("Specific Phenotypic Traits" = c("Apparent Digestability Coefficient", "Cortisol Levels", "Development Time", 
...
Raw_Acclimation_Specific_Trait_CVR <- data.frame("Specific Phenotypic Traits" = c("Apparent Digestability Coefficient", "Catalase Activity", "Cortisol Levels", "Food Consumption", 
...
Heterogeneity_Overall_CVR <- data.frame("Models" = c("Overall", "Fluctuation Amplitude", "Fluctuation Type", 
                                        ...
                                        "Shared Animal" = c(Overall_Model_CVR_i2[5, 1], Amplitude_Model_CVR_i2[5, 1], Fluctuation_Model_CVR_i2[5, 1], 
                                                            Trait_Model_CVR_i2[5, 1], Specific_Trait_Model_CVR_i2[5, 1], Class_Model_CVR_i2[5, 1]), 
                                        "Shared Control" = c(Overall_Model_CVR_i2[6, 1], Amplitude_Model_CVR_i2[6, 1], Fluctuation_Model_CVR_i2[6, 1], 
...
Heterogeneity_Population_CVR <- data.frame("Models" = c("Population-level Traits", "Fluctuation Amplitude", 
                                           ...
                                           "Shared Animal" = c(Population_Model_CVR_i2[5, 1], Population_Amplitude_Model_CVR_i2[5, 1], 
                                                               Population_Fluctuation_Model_CVR_i2[5, 1], Population_Class_Model_CVR_i2[5, 1]), 
                                           "Shared Control" = c(Population_Model_CVR_i2[6, 1], Population_Amplitude_Model_CVR_i2[6, 1], 
...
Heterogeneity_Individual_CVR <- data.frame("Models" = c("Individual-level Traits", "Fluctuation Amplitude", 
                                                        "Fluctuation Type", "Taxonomic Class"), 
                                           "Shared Animal" = c(Individual_Model_CVR_i2[5, 1], Individual_Amplitude_Model_CVR_i2[5, 1], 
                                                               Individual_Fluctuation_Model_CVR_i2[5, 1], Individual_Class_Model_CVR_i2[5, 1]), 
                                           "Shared Control" = c(Individual_Model_CVR_i2[6, 1], Individual_Amplitude_Model_CVR_i2[6, 1], 
...
Heterogeneity_Aquatic_CVR <- data.frame("Models" = c("Aquatic Organisms", "Exposure Type", "Fluctuation Amplitude", 
                                        ...
                                        "Shared Animal" = c(Aquatic_Model_CVR_i2[5, 1], Aquatic_Plasticity_Model_CVR_i2[5, 1], Aquatic_Amplitude_Model_CVR_i2[5, 1], 
                                                            Aquatic_Fluctuation_Model_CVR_i2[5, 1], Aquatic_Trait_Model_CVR_i2[5, 1], Aquatic_Specific_Trait_Model_CVR_i2[5, 1]), 
                                        "Shared Control" = c(Aquatic_Model_CVR_i2[6, 1], Aquatic_Plasticity_Model_CVR_i2[6, 1], Aquatic_Amplitude_Model_CVR_i2[6, 1], 
...
Heterogeneity_Terrestrial_CVR <- data.frame("Models" = c("Terrestrial Organisms", "Exposure Type", "Fluctuation Amplitude", 
                                                         "Fluctuation Type", "Phenotypic Trait Categories", "Specific Phenotypic Traits"), 
                                            "Shared Animal" = c(Terrestrial_Model_CVR_i2[5, 1], Terrestrial_Plasticity_Model_CVR_i2[5, 1], Terrestrial_Amplitude_Model_CVR_i2[5, 1], 
                                                                Terrestrial_Fluctuation_Model_CVR_i2[5, 1], Terrestrial_Trait_Model_CVR_i2[5, 1], Terrestrial_Specific_Trait_Model_CVR_i2[5, 1]), 
                                            "Shared Control" = c(Terrestrial_Model_CVR_i2[6, 1], Terrestrial_Plasticity_Model_CVR_i2[6, 1], Terrestrial_Amplitude_Model_CVR_i2[6, 1], 
...
Heterogeneity_Acclimation_CVR <- data.frame("Models" = c("Acclimation", "Exposure Time", "Fluctuation Amplitude", "Fluctuation Type", "Life-history Stage", 
                                            ...
                                            "Shared Animal" = c(Acclimation_Model_CVR_i2[5, 1], Acclimation_Exposure_Model_CVR_i2[5, 1], Acclimation_Amplitude_Model_CVR_i2[5, 1], Acclimation_Fluctuation_Model_CVR_i2[5, 1], Acclimation_Stage_Model_CVR_i2[5, 1], 
                                            ...
                                            "Shared Control" = c(Acclimation_Model_CVR_i2[6, 1], Acclimation_Exposure_Model_CVR_i2[6, 1], Acclimation_Amplitude_Model_CVR_i2[6, 1], Acclimation_Fluctuation_Model_CVR_i2[6, 1], Acclimation_Stage_Model_CVR_i2[6, 1], 
...
Heterogeneity_Developmental_CVR <- data.frame("Models" = c("Developmental", "Exposure Time", "Fluctuation Amplitude", "Fluctuation Type", 
                                                       "Phenotypic Trait Categories", "Specific Phenotypic Traits", "Taxonomic Class"), 
                                          "Shared Animal" = c(Developmental_Model_CVR_i2[5, 1], Developmental_Exposure_Model_CVR_i2[5, 1], Developmental_Amplitude_Model_CVR_i2[5, 1], Developmental_Fluctuation_Model_CVR_i2[5, 1], 
                                                              Developmental_Trait_Model_CVR_i2[5, 1], Developmental_Specific_Trait_Model_CVR_i2[5, 1], Developmental_Class_Model_CVR_i2[5, 1]), 
                                          "Shared Control" = c(Developmental_Model_CVR_i2[6, 1], Developmental_Exposure_Model_CVR_i2[6, 1], Developmental_Amplitude_Model_CVR_i2[6, 1], Developmental_Fluctuation_Model_CVR_i2[6, 1], 
