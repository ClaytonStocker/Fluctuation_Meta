...
Funnel_Plot <- funnel(Residuals_Model, yaxis = "seinv",
                      ...
                      pch = 21, back = "#D3DDEB", bg = "#183357", digits = c(0, 2))
box(lwd = 2)
...
system.time( #  34ish minutes
  if(run){
    Cooks_Overall_Model <- metafor::rma.mv(InRR_Transformed ~ 1, V = v_InRR, test = "t", dfs = "contain",
                                           ...
                                           R = list(phylo=A_cor), data = data, method = "REML", sparse = TRUE, 
                                           control=list(rel.tol=1e-9))
    saveRDS(Cooks_Overall_Model, "./3.Data_Analysis/2.Outputs/Models/Cooks_Overall_Model.rds")
  } else {
    Cooks_Overall_Model <- readRDS("./3.Data_Analysis/2.Outputs/Models/Cooks_Overall_Model.rds")})
...
system.time( #  34ish minutes
  if(run){
    Cooks_Overall_Model_CVR <- metafor::rma.mv(InCVR ~ 1, V = v_InCVR, test = "t", dfs = "contain",
                                               ...
                                               R = list(phylo=A_cor), data = data, method = "REML", sparse = TRUE, 
                                               control=list(rel.tol=1e-9))
    saveRDS(Cooks_Overall_Model_CVR, "./3.Data_Analysis/2.Outputs/Models/Cooks_Overall_Model_CVR.rds")
  } else {
    Cooks_Overall_Model_CVR <- readRDS("./3.Data_Analysis/2.Outputs/Models/Cooks_Overall_Model_CVR.rds")})
